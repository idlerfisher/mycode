...
string CMd5::Md5(const string &src)
{
	char res[17];
	MD5Init(&m_md5Ctx);
	MD5Update(&m_md5Ctx, (unsigned char *)(src.c_str()), src.size());
	MD5Final(&m_md5Ctx, (unsigned char *)res);
	res[16] = 0;

    //res可能有空字符，必须要指定数量
	return ToHex(string(res, 16));
}

bool CMd5::IsMd5Data(const string &md5)
{
    return (32 == md5.size());
}

string CMd5::GetMd5(const string &src)
{
    CMd5 md5;
    return md5.Md5(src);
}
...
string CMd5::ToHex(const string &str)
{
	int j = 0;
	int size = str.size();
	string hex(size * 2, 0);
...
string CMd5::FromHex(const string &hex)
{
